...
do
	...
	do
		...
		do
			...
			python activation_maximisation.py --output_dir $Adam_path --init_lr $lr --reg_param $rp --n_iters 1000 --param_file 'params_dump_Adam.txt' --neuron $neuron_idx --n_out_neuron 2 --layer 'fc9'

			if [ $sgd_optm -eq 1 ]
			then
				mkdir results/SGD/neuron$neuron_idx/$lr_str$lr$rp_str$rp
				mkdir results/SGD/neuron$neuron_idx/$lr_str$lr$rp_str$rp/examples
				python activation_maximisation.py --output_dir $SGD_path --optimizer 'SGD' --init_lr $lr --reg_param $rp --n_iters 1000 --param_file 'params_dump_SGD.txt' --neuron $neuron_idx --n_out_neuron 2 --layer 'fc9'
			fi
